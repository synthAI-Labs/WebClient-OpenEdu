...
const RenderContent = ({ contentURL }: { contentURL: string }) => {
  ...
  const customComponents = {
    ...
    blockquote: ({ children }: { children: React.ReactNode }) => (
      <blockquote className="border-l-4 border-gray-300 italic my-4 pl-4">
        {children}
      </blockquote>
    ),
    ...
    inlineCode: ({ children }: { children: React.ReactNode }) => (
      <code className="bg-gray-200 px-1 rounded-md">{children}</code>
    ),
    code: ({ children }: { children: React.ReactNode }) => (
      <code className="bg-gray-200 px-1 rounded-md">{children}</code>
    ),
    pre: ({ children }: { children: React.ReactNode }) => (
      <pre className="bg-gray-200 px-1 rounded-md">{children}</pre>
    ),
    img: ({ src, alt }: { src: string; alt: string }) => (
      <img className="my-4" src={src} alt={alt} />
    ),
  };
